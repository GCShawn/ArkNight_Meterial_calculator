...
third_metrial = ['D32钢','双极纳米片','聚合剂']
second_meterial = ['RMA70-24','五水研磨石','三水锰矿','白马醇','酮阵列','改量装置','糖聚块','聚酸酯块','提纯源岩','异铁块']
first_meterial = ['酮凝集组','全新装置','异铁组','聚酸酯组','糖组','固源岩组','RAM70-12','研磨石','扭转醇','轻锰矿']
based_meterial = ['酮凝集','装置','异铁','聚酸酯','糖','固源岩']
#gray_meterial = ['双酮','破损装置','异铁碎片','酯原料','糖','源岩']

class Meterial:
    meterial_name = ''
    sub_meterial_and_number = []
    cost = 0

    def __init__(self,name,sub_meterial,cost):
        self.meterial_name = name
        self.sub_meterial_and_number = sub_meterial
        self.cost = cost

    def show(self):
        print(self.meterial_name)
        print(self.sub_meterial_and_number)
        print(self.cost)

    def sort(self):
        temp1 = self.sub_meterial_and_number[:]
        temp2 = []
        for i in temp1:
        ...
        while j<=l:
            count = 0
            while (temp2[j] in self.sub_meterial_and_number):

                index = self.sub_meterial_and_number.index(temp2[j])+1
                count = count + self.sub_meterial_and_number[index]
                del self.sub_meterial_and_number[index-1]
            ...
            if j>=l:
                break
        self.sub_meterial_and_number = temp2[:]

def made_by(name,*sub_meterial_meterial_number,cost):
    New_meterial = []
    New_cost = 0

    i=0
    while i <= (len(sub_meterial_meterial_number)-2):
        times = sub_meterial_meterial_number[i+1]
        for j in range(times):
            New_cost = New_cost + sub_meterial_meterial_number[i].cost
            New_meterial.extend(sub_meterial_meterial_number[i].sub_meterial_and_number)
        i+=2

    New_cost +=cost
    return_meterial = Meterial(name,New_meterial,New_cost)

    return_meterial.sort()

    return return_meterial


def main():

    #based_meterial = [0'酮凝集',1'装置',2'异铁',3'聚酸酯',4'糖',5'固源岩',6'RAM70-12', 7'研磨石', 8'扭转醇', 9'轻锰矿']
    Ketone_agglutination = Meterial(based_meterial[0],['酮凝集',1],0)
    Device = Meterial(based_meterial[1],['装置',1],0)
    Iso_iron = Meterial(based_meterial[2],['异铁',1],0)
    Polyester = Meterial(based_meterial[3],['聚酸酯',1],0)
    Sugar = Meterial(based_meterial[4],["糖",1],0)
    Solid_source_stone = Meterial(based_meterial[5],["固源岩",1],0)
    RMA7012 = Meterial(first_meterial[6],['RMA70-12',1],0)
    Grinding_stone = Meterial(first_meterial[7], ['研磨石', 1], 0)
    Reverse_alcohol = Meterial(first_meterial[8], ['扭转醇', 1], 0)
    Light_manganese_ore = Meterial(first_meterial[9], ['轻锰矿', 1], 0)
